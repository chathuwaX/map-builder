...
try:
    ...
    print("Warning: Hardware libraries (board, busio, digitalio) not found. Switching to Mock mode.")
...
def _display_loop():
    ...
    if not DISPLAY_AVAILABLE and PYGAME_AVAILABLE:
        ...
        try:
            pygame.init()
            # Wider window for portrait dual view: Margin(20) + Eye(128) + Gap(140) + Eye(128) + Margin(20) = 436
            _preview_screen = pygame.display.set_mode((436, SCREEN_HEIGHT + 60))
            pygame.display.set_caption("Voice Agent Eyes - Preview (Portrait)")
