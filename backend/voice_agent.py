...
class CampusGreetingAgent(Agent):
    ...
    async def show_location_map(self, location_query: str, context: RunContext) -> str:
        """Displays a campus location map on the frontend. Use this when someone asks for a map or to show a location."""
        print(f"🗺️ [TOOL] show_location_map redirected to get_directions for: {location_query}")
        return await self.get_directions(location_query, context)
