...
export default function NavigationViewer() {
  const [mapData, setMapData] = useState<{ nodes: any[], buildings: any, edges: any[] } | null>(null);
  ...
  return (
    <div className="w-full h-screen relative flex flex-col bg-[#0F172A]">
      <div className="absolute top-4 left-4 z-10 bg-slate-800/80 p-3 rounded-xl border border-slate-700 backdrop-blur-sm shadow-xl flex items-center gap-3">
        <span className="text-slate-300 font-medium text-sm">Select Floor:</span>
        <select 
      ...
      {!mapData ? (
        <div className="w-full flex-1 flex items-center justify-center text-slate-400">Loading floor data...</div>
      ) : (
