...
export default function PathEditorPage() {
  ...
  // ── History ───────────────────────────────────────────────────────
  const [hist, setHist] = useState<{ stack: FloorData[]; index: number }>({
    stack: [],
    index: -1,
  });
      ...
      setHist((prev) => {
        ...
        if (
          ...
          JSON.stringify({
            n: lastState.nodes,
            e: lastState.edges,
            b: lastState.buildings,
          }) ===
            JSON.stringify({
              n: nextState.nodes,
              e: nextState.edges,
              b: nextState.buildings,
            })
        ) {
          return prev;
        }
  ...
  const bbox = useMemo(() => {
    ...
    bVals.forEach((b) => {
      const hw = b.size[0] / 2,
        hh = b.size[1] / 2;
      minX = Math.min(minX, b.position[0] - hw);
      maxX = Math.max(maxX, b.position[0] + hw);
      minZ = Math.min(minZ, b.position[2] - hh);
      maxZ = Math.max(maxZ, b.position[2] + hh);
    });
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      setSaveMsg({
        ok: true,
        text: `Floor ${floor.replace("floor_", "")} saved! ${nodes.filter((n) => n.type === "waypoint").length} waypoints, ${edges.length} edges.`,
      });
  ...
  const handleClearNav = () => {
    if (
      !confirm(
        `Remove ALL waypoints and edges from ${floor.replace("floor_", "Floor ")}? Rooms stay. You cannot undo unless you reload.`,
      )
    )
      return;
      ...
      if (mode === "shape_building") {
        ...
        if (col >= 0 && col < b.size[0] && row >= 0 && row < b.size[1]) {
          ...
          setBuildings((prev) => {
            ...
            const newCells = cells.includes(cellId)
              ? cells.filter((c) => c !== cellId)
              : [...cells, cellId];
            return {
              ...prev,
              [clickedBuilding!]: { ...bPrev, removed_cells: newCells },
            };
          });
        }
        return;
  ...
  const onWheel = (e: React.WheelEvent) => {
    e.preventDefault();
    setZoom((z) =>
      Math.max(0.4, Math.min(6, e.deltaY < 0 ? z * 1.12 : z / 1.12)),
    );
  };
  ...
  const onMouseMove = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    if (dragNodeRef.current) {
      ...
      setNodes((prev) =>
        prev.map((n) =>
          n.id === dragNodeRef.current ? { ...n, x: nx, z: nz } : n,
        ),
      );
      return;
    }
  ...
  // ── Computed ──────────────────────────────────────────────────────
  const rooms = useMemo(
    () => nodes.filter((n) => n.type !== "waypoint"),
    [nodes],
  );
    ...
    >
      ...
      >
        ...
        <span className="text-[11px]" style={{ color: COLORS.muted }}>
          {rooms.length} rooms · {waypoints.length} waypoints · {edges.length}{" "}
          edges
        </span>
      ...
      {saveMsg && (
        <div
          className="px-4 py-2 flex items-center gap-2 text-xs font-medium shrink-0"
          style={{
            background: saveMsg.ok ? "#052E16" : "#450A0A",
      ...
      {/* Mode hint */}
      <div
        className="px-4 py-1.5 text-[11px] shrink-0 flex items-center gap-3"
        style={{
          background: "#0F172A",
          color: COLORS.muted,
      ...
      >
        {mode === "select" && (
          <div className="flex items-center gap-4">
            <span>
              ✋ Click an item to select it. Drag a waypoint to reposition.
              Click an edge to delete it.
            </span>
            {selectedId &&
              nodes.find((n) => n.id === selectedId)?.type !== "waypoint" && (
                <div className="flex items-center gap-3 ml-4 border-l border-slate-700 pl-4">
                  <span className="font-bold text-white">
                    {nodes.find((n) => n.id === selectedId)?.label} Size:
                  </span>
                  <label className="flex items-center gap-1">
                    ...
                    <input
                      ...
                      max="20"
                      value={
                        nodes.find((n) => n.id === selectedId)?.size?.[0] || 1
                      }
                      onChange={(e) => {
                  ...
                  <label className="flex items-center gap-1">
                    ...
                    <input
                      ...
                      max="20"
                      value={
                        nodes.find((n) => n.id === selectedId)?.size?.[2] || 1
                      }
                      onChange={(e) => {
                        const v = Number(e.target.value) || 1;
                        setNodes((prev) =>
        ...
        {mode === "move" &&
          "✥ Click and drag any room, waypoint, or building to move it. Release to place. Edges follow automatically."}
        {mode === "add_waypoint" &&
          "📍 Click anywhere inside a building to place a new waypoint. Place them along corridors and at junctions."}
        {mode === "connect" &&
          (connectFrom
        ...
        {mode === "delete" &&
          "🗑 Click any waypoint or edge to delete it. Rooms cannot be deleted."}
        {mode === "shape_building" && (
          <div className="flex items-center gap-4">
            <span>
              🏗 Click on a building to select it. Click grid cells to
              remove/add them.
            </span>
            {selectedBuildingId && buildings[selectedBuildingId] && (
              <div className="flex items-center gap-3">
                <span className="font-bold text-white">
                  {buildings[selectedBuildingId].name}
                </span>
                <label className="flex items-center gap-1">
                  Width:
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setBuildings((p) => ({
                        ...p,
                        [selectedBuildingId]: {
                          ...p[selectedBuildingId],
                          size: [v, p[selectedBuildingId].size[1]],
                        },
                      }));
                    }}
                    className="w-16 bg-slate-800 px-1 py-0.5 rounded outline-none border border-slate-600 text-white"
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setBuildings((p) => ({
                        ...
                        [selectedBuildingId]: {
                          ...p[selectedBuildingId],
                          size: [p[selectedBuildingId].size[0], v],
                        },
                      }));
                    }}
                    className="w-16 bg-slate-800 px-1 py-0.5 rounded outline-none border border-slate-600 text-white"
        ...
        {connectFrom && (
          <span className="ml-auto font-bold" style={{ color: "#6366F1" }}>
            Connecting from:{" "}
            {nodes.find((n) => n.id === connectFrom)?.label ?? connectFrom}
          </span>
        )}
      </div>
      ...
      <div className="flex-1 overflow-hidden relative">
        {loading ? (
          <div
            className="w-full h-full flex items-center justify-center text-sm animate-pulse"
                ...
                return (
                  ...
                  >
                    ...
                    <path
                      d={[
                        ...Array.from(
                          { length: Math.round(b.size[0]) + 1 },
                          (_, i) => `M ${sx + i * scale} ${sy} v ${bh}`,
                        ),
                        ...Array.from(
                          { length: Math.round(b.size[1]) + 1 },
                          (_, i) => `M ${sx} ${sy + i * scale} h ${bw}`,
                        ),
                      ].join(" ")}
                      fill="none"
                      stroke="#0F172A"
