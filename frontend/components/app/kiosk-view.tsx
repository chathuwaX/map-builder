...
import { Track, DataPacket_Kind, RemoteParticipant } from "livekit-client";
import React, {
  useState,
  useEffect,
  useRef,
...
export function KioskView() {
    ...
    const handleDataReceived = (
      ...
      try {
        ...
        if (message.type === "navigation") {
          console.log(
            "🗺️ KioskView received navigation data:",
            message.destination,
          );
          setNavData(message);
        }
      } catch (error) {
        console.error("❌ Error parsing data message in KioskView:", error);
      }
    };
  ...
  return (
    <div
      className="relative text-on-background w-full h-screen overflow-hidden flex flex-col select-none bg-background/40"
    ...
    >
      ...
      <div className="absolute inset-0 -z-20 overflow-hidden pointer-events-none bg-background transition-colors duration-1000">
        <div
          ...
          style={{
            background:
              fbPosts[currentSlide]?.category === "competitions"
                ? "radial-gradient(circle, rgba(249,115,22,0.18) 0%, rgba(244,63,94,0.06) 70%, rgba(255,255,255,0) 100%)"
                : fbPosts[currentSlide]?.category === "events"
                  ? "radial-gradient(circle, rgba(139,92,246,0.15) 0%, rgba(99,102,241,0.05) 70%, rgba(255,255,255,0) 100%)"
                  : fbPosts[currentSlide]?.category === "posts"
                    ? "radial-gradient(circle, rgba(20,184,166,0.15) 0%, rgba(6,182,212,0.05) 70%, rgba(255,255,255,0) 100%)"
        ...
        <div
          ...
          style={{
            background:
              fbPosts[currentSlide]?.category === "competitions"
                ? "radial-gradient(circle, rgba(244,63,94,0.15) 0%, rgba(249,115,22,0.05) 70%, rgba(255,255,255,0) 100%)"
                : fbPosts[currentSlide]?.category === "events"
                  ? "radial-gradient(circle, rgba(99,102,241,0.15) 0%, rgba(139,92,246,0.05) 70%, rgba(255,255,255,0) 100%)"
                  : fbPosts[currentSlide]?.category === "posts"
                    ? "radial-gradient(circle, rgba(6,182,212,0.15) 0%, rgba(20,184,166,0.05) 70%, rgba(255,255,255,0) 100%)"
                    : "radial-gradient(circle, rgba(78,96,118,0.12) 0%, rgba(78,96,118,0.03) 70%, rgba(255,255,255,0) 100%)",
      ...
      `}</style>
        ...
        <main className="flex-1 pl-8 pr-4 pt-3 pb-4 overflow-hidden min-h-0 flex flex-col">
          <div className="flex gap-6 flex-1 min-h-0 pb-2 relative">
            ...
            >
              ...
              <div className="bg-primary-container text-on-primary-container rounded-3xl p-5 pt-8 flex flex-col items-center justify-center shadow-sm relative overflow-hidden flex-shrink-0">
                ...
                <div className="text-[44px] leading-[44px] tracking-[-0.04em] font-bold text-primary">
                  {time || "10:42"}
                </div>
                <div className="text-[14px] leading-[20px] mt-1 font-bold opacity-90">
              ...
              <div className="bg-surface-container rounded-3xl p-5 shadow-sm flex-1 flex flex-col relative overflow-hidden min-h-0">
                ...
                <div className="flex flex-col gap-3 mt-auto w-full">
                  <button
                    onClick={() =>
                      handleCategoryClick("Lecture Halls", "lecture")
                    }
                    className="bg-primary text-on-primary rounded-full h-[50px] w-full text-[17px] flex items-center justify-center gap-3 hover:bg-surface-tint transition-colors active:scale-95 shadow-md font-bold flex-shrink-0"
                  >
                  ...
                  <button
                    ...
                    className="bg-surface-variant text-on-surface-variant rounded-full h-[50px] w-full text-[17px] flex items-center justify-center gap-3 hover:bg-surface-container-highest transition-colors active:scale-95 shadow-sm border border-outline-variant font-bold flex-shrink-0"
                  >
                    <span className="material-symbols-outlined text-2xl">
                      science
                    </span>
                  ...
                  <button
                    onClick={() =>
                      handleCategoryClick("Auditoriums", "auditorium")
                    }
                    className="bg-surface-variant text-on-surface-variant rounded-full h-[50px] w-full text-[17px] flex items-center justify-center gap-3 hover:bg-surface-container-highest transition-colors active:scale-95 shadow-sm border border-outline-variant font-bold flex-shrink-0"
                  >
                    <span className="material-symbols-outlined text-2xl">
            ...
            <div className="flex-1 h-full min-h-0 flex flex-col gap-6 min-w-0">
              ...
              <div
                className={`flex-shrink-0 min-h-[96px] h-auto py-4 flex items-center justify-center rounded-3xl shadow-sm relative px-4 overflow-hidden transition-all duration-700 ${isConnected ? "bg-surface-container border border-primary/20" : "bg-surface-container-low"}`}
              >
                {/* Gemini-style Wavy Gradient Background */}
                <div
                  className={`absolute inset-0 overflow-hidden pointer-events-none transition-all duration-300 ease-in-out ${isThinking ? "animate-pulse" : ""} ${!isConnected ? "animate-breathe" : ""}`}
                  style={
                    isConnected
                      ? {
              ...
              <AnimatePresence mode="wait">
                {isNavigating || isMapExpanded ? (
                  <motion.div
                    ...
                    className="bg-surface-container text-on-surface rounded-3xl shadow-md h-full flex flex-col border border-outline/20 min-h-0 overflow-hidden relative pt-4"
                  >
                    <div className="px-4 pb-2 border-b border-outline/10 flex items-center justify-between flex-shrink-0">
                    ...
                    <ScrollArea
                      ref={scrollAreaRef}
                      className="flex-1 px-4 relative z-10 pt-4"
                    >
                      <ChatTranscript
                        messages={messages}
                        transcriptions={transcriptions}
                  ...
                  <motion.div
                    ...
                    exit={{ opacity: 0, x: -20 }}
                    transition={{ duration: 0.3 }}
                    className="bg-primary text-on-primary rounded-3xl shadow-md h-full flex flex-col border-4 border-primary-container/30 min-h-0 overflow-hidden relative"
                  >
                    {focusedEvent ? (
                      /* Full poster view */
                      <>
                        ...
                        <div className="flex-shrink-0 p-4 bg-primary">
                          <p className="text-on-primary font-bold text-[16px] leading-snug mb-1">
                            {focusedEvent.message}
                          </p>
                          {focusedEvent.description && (
                            <p className="text-on-primary/75 text-[12px] leading-relaxed line-clamp-3 mb-2">
                              {focusedEvent.description}
                            </p>
                          )}
              ...
              <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
                {filteredLocations.length > 0 ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-12 opacity-50">
                    ...
                    <p className="text-lg">
                      No {locationsModalCategory.toLowerCase()} found.
                    </p>
                  </div>
                )}
              </div>
